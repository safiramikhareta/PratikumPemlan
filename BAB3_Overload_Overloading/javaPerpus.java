package BAB3_Overload_Overloading;
public class javaPerpus
{
    public static void main(String[] args) 
    {
        perpus b1 = new perpus();
        b1.setJudul("A History of science technology and philosophy in the eighteenth century");
        b1.setKategori("Teknologi");
        b1.setPenulis("McKie, D.");
        b1.setPenerbitan("LondonGeorge Allen & Unwin Ltd 1952");
        b1.setBahasa("Inggris");
        b1.setEdisi("Kedua");
        b1.setHalaman(814);
        b1.setSinopsis("Buku ini menguraikan penjelasan runtutan peristiwa sejarah dan filosofi mengenai teknologi");
        b1.setPenjualan(21);
        b1.hitungRoyalti(348700);
        b1.displayMessage();
        System.out.println("=====================================");
    
        perpus b2 = new perpus();
        b2.setJudul("Information and Communication Technology, dan Literasi Media Digital");
        b2.setKategori("Teknologi");
        b2.setPenulisbanyak(new String[]{"Dewi Kartika Sari", "Nurul Hasfi", "Hedi Pudjo Santosa", "Triyono Lukmantoro", "Narayana Mahendra Prastya", "Santi Isnaini", "Said Romadlan", "Sa’diyah El Adawiyah", "Suwarto", "Aminah Swarnawati", "AgungPrabowo", "Kurnia Arofah", "Sri Budi Lestari", "Edi antoso","M as’Amah"});
        b2.setPenerbitan("ASPIKOM 2015");
        b2.setBahasa("Indonesia");
        b2.setEdisi("Pertama");
        b2.setHalaman(178);
        b2.setSinopsis("Buku ini menjelaskan mengenai literasi media digital dan teknologi informasi serta komunikasi");
        b2.setPenjualan(30);
        b2.hitungRoyalti(60000);
        b2.displayMessage();
        System.out.println("=====================================");

        perpus b3 = new perpus();
        b3.setJudul("Dunia Sophie");
        b3.setKategori("Filsafat");
        b3.setPenulis("Jostein Gaarder");
        b3.setPenerbitan("Mizan Pustaka 2014");
        b3.setBahasa("Indonesia");
        b3.setEdisi("-");
        b3.setHalaman(800);
        b3.setSinopsis("Sebuah novel filsafat yang mengajarkan konsep dan sejarah filsafat melalui perjalanan Sophie, seorang gadis remaja yang mendapatkan surat-surat misterius tentang filsafat dan eksistensi");
        b3.setPenjualan(25);
        b3.hitungRoyalti(125000);
        b3.displayMessage();
        System.out.println("=====================================");

        perpus b4 = new perpus();
        b4.setJudul("Filsafat Kata");
        b4.setKategori("Filsafat");
        b4.setPenulis("Reza Alexander Antonius Wattimena");
        b4.setPenerbitan("Evolitera 2011");
        b4.setBahasa("Indonesia");
        b4.setEdisi("-");
        b4.setHalaman(396);
        b4.setSinopsis("Buku yang membahas pentingnya kata-kata dalam bahasa dan pemikiran manusia serta perannya dalam membentuk realitas dan konsep filosofis yang ada");
        b4.setPenjualan(15);
        b4.hitungRoyalti(50000);
        b4.displayMessage();
        System.out.println("=====================================");

        perpus b5 = new perpus();
        b5.setJudul("Dinamika Pendidikan Islam: Perspektif Historis");
        b5.setKategori("Sejarah");
        b5.setPenulis("Dr. Siswanto, M.Pd.I");
        b5.setPenerbitan("Pena Salsabila 2013");
        b5.setBahasa("Indonesia");
        b5.setEdisi("-");
        b5.setHalaman(190);
        b5.setSinopsis("Buku yang mengungkapkan perjalanan sejarah pendidikan Islam dari masa ke masa");
        b5.setPenjualan(10);
        b5.hitungRoyalti(45000);
        b5.displayMessage();
        System.out.println("=====================================");

        perpus b6 = new perpus();
        b6.setJudul("Penulisan dan Penghimpunan Hadis: Kajian Historis");
        b6.setKategori("Sejarah");
        b6.setPenulis("Rasul Ja'fariyan");
        b6.setPenerbitan("Lentera 1992");
        b6.setBahasa("Indonesia");
        b6.setEdisi("Pertama");
        b6.setHalaman(100);
        b6.setSinopsis("Buku yang mengulas tentang bagaimana hadis-hadis dalam Islam ditulis dan dikumpulkan dari masa ke masa");
        b6.setPenjualan(18);
        b6.hitungRoyalti(47000);
        b6.displayMessage();
        System.out.println("=====================================");

        perpus b7 = new perpus();
        b7.setJudul("Berkata Baik atau Diam: 294 Adab Kebiasaan Rasulullah SAW.");
        b7.setKategori("Agama");
        b7.setPenulis("Ninik Handrini");
        b7.setPenerbitan("PT. Gramedia Pustaka Utama 2016");
        b7.setBahasa("Indonesia");
        b7.setEdisi("Pertama");
        b7.setHalaman(240);
        b7.setSinopsis("Buku yang memberikan penjelasan tentang berbagai adab dan kebiasaan Rasulullah SAW");
        b7.setPenjualan(12);
        b7.hitungRoyalti(65000);
        b7.displayMessage();
        System.out.println("=====================================");

        perpus b8 = new perpus();
        b8.setJudul("Ya Allah, Aku Jatuh Cinta");
        b8.setKategori("Agama");
        b8.setPenulis("Qowi Alta Az Zahra");
        b8.setPenerbitan("PT. Elex Media Komputindo 2014");
        b8.setBahasa("Indonesia");
        b8.setEdisi("Pertama");
        b8.setHalaman(116);
        b8.setSinopsis("Buku yang membahas kisah-kisah inspiratif dan pengalaman spiritual dalam mencintai Allah SWT");
        b8.setPenjualan(20);
        b8.hitungRoyalti(28000);
        b8.displayMessage();
        System.out.println("=====================================");

        perpus b9 = new perpus();
        b9.setJudul("Akhir Bahagia");
        b9.setKategori("Psikologi");
        b9.setPenulis("Sandy Oogway");
        b9.setPenerbitan("PT. Karpindo 2018");
        b9.setBahasa("Indonesia");
        b9.setEdisi("Pertama");
        b9.setHalaman(121);
        b9.setSinopsis("Buku psikologi yang mengajarkan strategi dan keterampilan untuk menciptakan kebahagiaan dalam hidup");
        b9.setPenjualan(30);
        b9.hitungRoyalti(60000);
        b9.displayMessage();
        System.out.println("=====================================");

        perpus b10 = new perpus();
        b10.setJudul("Modul Pembelajaran dan Praktikum Tes Psikologi");
        b10.setKategori("Psikologi");
        b10.setPenulis("Zainul Anwar");
        b10.setPenerbitan("Psychology Forum 2021");
        b10.setBahasa("Indonesia");
        b10.setEdisi("-");
        b10.setHalaman(60);
        b10.setSinopsis("Buku yang memberikan panduan praktis tentang teori dan aplikasi tes psikologi serta modul latihan dan kasus studi");
        b10.setPenjualan(35);
        b10.hitungRoyalti(95000);
        b10.displayMessage();
        System.out.println("=====================================");

        perpus b11 = new perpus();
        b11.setJudul("Nasionalisme Indonesia Kini dan di Masa Depan");
        b11.setKategori("Politik");
        b11.setPenulis("Benedict Anderson");
        b11.setPenerbitan("Anjing Galak Penerbitan 2010");
        b11.setBahasa("Indonesia");
        b11.setEdisi("-");
        b11.setHalaman(42);
        b11.setSinopsis("Sebuah buku yang membahas peran dan tantangan nasionalisme di Indonesia");
        b11.setPenjualan(30);
        b11.hitungRoyalti(20000);
        b11.displayMessage();
        System.out.println("=====================================");
        
        perpus b12 = new perpus();
        b12.setJudul("Nyanyi sunyi seorang bisu: Catatan-catatan dari P. Buru");
        b12.setKategori("Politik");
        b12.setPenulis("Pramoedya Ananta Toer");
        b12.setPenerbitan("Lentera 1995");
        b12.setBahasa("Indonesia");
        b12.setEdisi("Pertama");
        b12.setHalaman(316);
        b12.setSinopsis("Buku yang berisi catatan pengalaman naratif dari seorang tahanan politik Indonesia, Pramoedya Ananta Toer selama dipenjara");
        b12.setPenjualan(40);
        b12.hitungRoyalti(75000);
        b12.displayMessage();
        System.out.println("=====================================");
        
        perpus b13 = new perpus();
        b13.setJudul("Laut Bercerita");
        b13.setKategori("Fiksi");
        b13.setPenulis("Leila S. Chudori");
        b13.setPenerbitan("Gramedia Pustaka Utama 2017");
        b13.setBahasa("Indonesia");
        b13.setEdisi("-");
        b13.setHalaman(390);
        b13.setSinopsis("Novel fiksi yang mengisahkan perjalanan seorang wanita dalam menemukan jati dirinya dan melepas masa lalunya melalui petualangan spiritual di sebuah pulau terpencil");
        b13.setPenjualan(1000);
        b13.hitungRoyalti(95000);
        b13.displayMessage();
        System.out.println("=====================================");
        
        perpus b14 = new perpus();
        b14.setJudul("Seperti Dendam, Rindu Harus Dibayar Tuntas");
        b14.setKategori("Fiksi");
        b14.setPenulis("Eka Kurniawan");
        b14.setPenerbitan("Gramedia Pustaka Utama 2019");
        b14.setBahasa("Indonesia");
        b14.setEdisi("-");
        b14.setHalaman(257);
        b14.setSinopsis("");
        b14.setPenjualan(40);
        b14.hitungRoyalti(79000);
        b14.displayMessage();
        System.out.println("=====================================");

        perpus b15 = new perpus();
        b15.setJudul("Dilan 1990");
        b15.setPenulis("Pidi Baiq");
        b15.simpanFile("C:/Users/HP/Documents/Pemrograman Lanjut/OOP/src/PratikumTugas.txt");

        perpus b16 = new perpus();
        b16.bacaFile("C:/Users/HP/Documents/Pemrograman Lanjut/OOP/src/PratikumTugas.txt");
        System.out.println("Judul : " +b16.getJudul());
        System.out.println("Penulis : " +b16.getPenulis());
        System.out.println("=====================================");

        System.out.println("Tingkat Kesamaan : " + b1.cekTingkatKesamaan(b4) + "%");
        System.out.println("Tingkat Kesamaan : " + b6.cekTingkatKesamaan(b7) + "%");
        System.out.println("Tingkat Kesamaan : " + b5.cekTingkatKesamaan(b8) + "%");
    }
}